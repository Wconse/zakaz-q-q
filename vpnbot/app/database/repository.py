...
from app.database.models import (
    BroadcastLog, Payment, PaymentStatus, PaymentSystem,
    PlanType, PromoCode, PromoUse, Subscription, SubscriptionStatus, User,
)
...
class UserRepository:
    ...
    async def mark_unblocked(self, telegram_id: int) -> None:
        await self._s.execute(
            update(User)
            .where(User.telegram_id == telegram_id)
            .values(blocked_bot_at=None)
        )

    async def set_remnawave_uuid(self, telegram_id: int, uuid: str) -> None:
        await self._s.execute(
    ...
    async def get_referrals(self, telegram_id: int) -> list[User]:
        ...
        r = await self._s.execute(
            select(User).where(User.referred_by == telegram_id)
        )
        return list(r.scalars().all())
    ...
    async def delete(self, telegram_id: int) -> None:
        """Delete user but keep trial_used flag — store in separate query to preserve it."""
