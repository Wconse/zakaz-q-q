...
def admin_grant_plan_keyboard(telegram_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="🎁 Пробный (trial)", callback_data=f"admin:grant:plan:trial:{telegram_id}")],
            [InlineKeyboardButton(text="📦 Стандартный", callback_data=f"admin:grant:plan:standard:{telegram_id}")],
            [InlineKeyboardButton(text="🚀 Расширенный", callback_data=f"admin:grant:plan:extended:{telegram_id}")],
